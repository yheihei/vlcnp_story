...
ウィンドウは1320×980、動画・静止画の領域は従来の960×540です。見出し60・説明文38の文字サイズを維持し、文字領域と下部の余白を調整しています。下部には左右のページ切替案内、ページ数、アイボリーの決定案内を配置しました。外枠を四方へ40ずつ広げ、下段の案内と外枠の間には60の余白があります。CanvasScalerは画面の高さを基準に拡縮し、横長画面でも上下が切れないようにしています。動画再生、スライドデータ、入力処理、閉じた後のコールバックは既存のものを使います。

見出し下のTitleAccentと下段上のFooterDividerは非表示です。見出しの文字サイズと余白で内容を区切っています。
