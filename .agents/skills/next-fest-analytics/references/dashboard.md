...
2026-09-07 の運用記録では Event Browser は1〜2分、Data Explorer は約10分で表示されたことがある一方、Funnels の当日分は表示されなかった。これを保証時間や「当日分は常に出ない」という規則にしない。現画面の更新時刻と期間を確認し、反映待ちを判別できなければ未確定と報告する。自動の再確認・通知は依頼された場合にだけ設定する。
...
旧運用では New Experience の一覧にイベントが出ず、Old Experience を使ったことがある。現在も切り替えがあり、それで必要な集計ができる場合だけ利用する。新レポートの保存は依頼範囲に含まれる場合に行う。
