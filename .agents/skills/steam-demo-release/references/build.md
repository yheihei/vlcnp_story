...
```sh
unity command menu --path "Tools/VLCNP/Build/Steam Demo Windows Release Build" --caller plugin --skill steam-demo-release --format json --timeout 1800
```

Windows 完了後、macOS を必要とする場合に実行する。同じ Editor で両ビルドを並列起動しない。

```sh
unity command menu --path "Tools/VLCNP/Build/Steam Demo macOS Release Build" --caller plugin --skill steam-demo-release --format json --timeout 1800
```
